#!/usr/bin/env python3
"""
Suicide Severity DPRF Evaluation Script (DPRF2 Version)

This script evaluates the DPRF framework on the Reddit Suicide Severity dataset,
...
sys.path.insert(0, os.path.dirname(evaluate_dir)) # Add parent of suicide dir (Evaluate dir)

from evaluate import BaseEvaluator
from core.utils import ensure_directory

class SuicideSeverityEvaluator(BaseEvaluator):
    """
    Evaluator for DPRF on the Reddit Suicide Severity dataset.
    Inherits from BaseEvaluator and handles suicide-specific data processing.
    """
    def __init__(self,
                 data_dir="Evaluation/suicide/data/processed", # Default relative to project root
        ...
        self.task_specific_id_keys_for_iteration_df = ["id", "suicide_level"]

    async def load_examples(self):
        """
        Load suicide severity examples from the processed data.
        """
        import random
        import time
        
        examples_path = os.path.join(self.data_dir, "suicide_data.json")
        if not os.path.exists(examples_path):
            raise FileNotFoundError(f"Suicide examples file not found: {examples_path}")

        with open(examples_path, "r") as f:
            all_examples = json.load(f)
        print(f"Loaded {len(all_examples)} total suicide severity examples from {examples_path}")
    ...
    async def create_task(self, example_data):
        """
        Process a raw suicide severity example into a structured task for BaseEvaluator.
        """
        # Get the example data
        post = example_data["post"]
        suicide_level = example_data["suicide_level"]
        initial_persona = example_data["initial_persona"]
        
        # Create content from the suicide risk level
        content = f"Your suicide risk level is: {suicide_level}"
        ...
        ground_truth = post
        bio_text = None # Suicide examples don't have bio text

        task_specific_info = {
            "id": example_data["id"],
            "suicide_level": suicide_level
        }
...
def main():
    parser = argparse.ArgumentParser(description="DPRF Evaluation for Suicide Severity (DPRF2)")
    ...
    wandb.init(
        ...
        name=f"{args.wandb_run_name}_suicide_{args.task_model.replace('/', '-')}",
        notes=args.wandb_notes or "Suicide severity evaluation run (DPRF2 Structure)",
        config={
    ...
    evaluator = SuicideSeverityEvaluator(
        data_dir=args.data_dir,
        ...
        # Construct a more specific run name for suicide
        wandb_run_name=f"suicide_{args.task_model.replace('/', '-')}",
        wandb_notes=args.wandb_notes or "Suicide severity evaluation run (DPRF2 Structure)"
    )
