#!/bin/bash
# Variant B (split): direct refinement, no analysis step, 80/20 train/val split
...
echo "=== Variant B (split): Direct refinement (interview, processed_val2) ==="
OUTPUT_DIR="results/variant_b_split_interview_${TIMESTAMP}"
mkdir -p "${OUTPUT_DIR}"
python Evaluation/interview/interview.py \
  ...
  --wandb_project dprf \
  --wandb_run_name variant_b_split

echo "=== Variant B (split): Direct refinement (debate, debate_variation.json) ==="
OUTPUT_DIR="results/variant_b_split_debate_${TIMESTAMP}"
mkdir -p "${OUTPUT_DIR}"
